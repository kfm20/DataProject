...
Duck.subset<- select(Duck, Month,TP:TC, OConcentration,TDS:MonthNumber)
#Need to add column of Pond Name if going to combine datasets into one
Duck.subset$PondName<- c("Duck")


South.subset<- select(South, Month,TP:TC, OConcentration,TDS:MonthNumber)
South.subset$PondName<- c("South")

Upper.subset<- select(Upper, Month,TP:TC, OConcentration,TDS:MonthNumber)
...
ggplot(DGRainWQ) +
  geom_point(aes(x = FecalColiform, 
                 y = OConcentration))+
  facet_wrap(vars(PondName), nrow=3)
